...
const MOCKED_CARDS_POOL_BY_LVL = populateCardsByLevelFromPool(MOCKED_CARDS_POOL)
...
describe('Game functionality', () => {
  ...
  it('can give tokens to player', () => {
    ...
    expect(game.table.tokens[ETokenColor.Blue]).toBe(TABLE_CONFIG[ETokenColor.Blue] - TOKENS_TO_TAKE[ETokenColor.Blue]);
    expect(game.table.tokens[ETokenColor.Red]).toBe(TABLE_CONFIG[ETokenColor.Red] - TOKENS_TO_TAKE[ETokenColor.Red]);
    expect(game.table.tokens[ETokenColor.Black]).toBe(TABLE_CONFIG[ETokenColor.Black] - TOKENS_TO_TAKE[ETokenColor.Black]);
    console.log(game.table.tokens);
  ...
  it('let player buy a card', () => {
    ...
    const game = new Game({
      ...GAME_CONFIG, players: [{
        name: 'max', id: FIRST_PLAYER.id, tokens: PLAYER_INITIAL_TOKENS
      }]
    });
    game.move();
    ...
    const CARD_TO_TAKE = MOCKED_CARDS_POOL_BY_LVL[EDevDeckLevel.First].slice(-1)[0];



    game.buyCardByPlayer(FIRST_PLAYER.id, EDevDeckLevel.First, 0);

    expect(game.getPlayer(FIRST_PLAYER.id).cardsBought[CARD_TO_TAKE.color][0].id).toBe(CARD_TO_TAKE.id);
    expect(game.table.First.cards.length === 4)
    expect(game.getPlayer(FIRST_PLAYER.id).tokens).toEqual({
      [ETokenColor.Black]: PLAYER_INITIAL_TOKENS[ETokenColor.Black] - (CARD_TO_TAKE.cost[ETokenColor.Black] || 0),
      [ETokenColor.Blue]: PLAYER_INITIAL_TOKENS[ETokenColor.Blue] - (CARD_TO_TAKE.cost[ETokenColor.Blue] || 0),
      [ETokenColor.Gold]: PLAYER_INITIAL_TOKENS[ETokenColor.Gold] - (CARD_TO_TAKE.cost[ETokenColor.Gold] || 0),
      [ETokenColor.Green]: PLAYER_INITIAL_TOKENS[ETokenColor.Green] - (CARD_TO_TAKE.cost[ETokenColor.Green] || 0),
      [ETokenColor.Red]: PLAYER_INITIAL_TOKENS[ETokenColor.Red] - (CARD_TO_TAKE.cost[ETokenColor.Red] || 0),
      [ETokenColor.White]: PLAYER_INITIAL_TOKENS[ETokenColor.White] - (CARD_TO_TAKE.cost[ETokenColor.White] || 0),
    })
  })
});
